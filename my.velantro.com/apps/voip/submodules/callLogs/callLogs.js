		...
		callLogsFormatCdrs: function(cdrs) {
			var self = this,
				...
				formatCdr = function(cdr) {
					...
					var recording_url = '';
					var recording_url_tip = '';
					if (cdr.hasOwnProperty('custom_channel_vars') && cdr.custom_channel_vars.hasOwnProperty('media_recording_id')) {
						recording_id = cdr.custom_channel_vars.media_recording_id;
						var tmp = recording_id.split('-', 2);
						var account_id = cdr.custom_channel_vars.account_id;
						var tmp_str = account_id.substr(0,2) + '%252F' + account_id.substr(2,2) + '%252F' + account_id.substr(4) + '-' + tmp[0];
						
						recording_url = 'https://s3-us-west-1.amazonaws.com/kazoorecordings/account%252F' + tmp_str + '/' + recording_id + '_' + cdr.custom_channel_vars.media_name;
						recording_url_tip = 'Listen';
					...
					var call = {
						...
						recording_url_tip: recording_url_tip,
